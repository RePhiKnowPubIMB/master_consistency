...
const url = 'https://youkn0wwho.academy/topic-list/segment_tree_with_lazy_propagation';
...
async function run() {
    ...
    // Filtering to only include actual problems
    const validProblems = problems.filter(p => 
        p.title !== 'NO_TITLE' && 
        p.difficulty !== 'UNKNOWN' &&
        (p.url.includes('codeforces') || p.url.includes('atcoder') || p.url.includes('spoj') || p.url.includes('lightoj') || p.url.includes('onlinejudge'))
    );

    console.log(`✅ Found ${validProblems.length} validated problems with tags.`);
    
    const outputPath = path.join(__dirname, '..', 'data', 'segment-tree-lazy-problems.json');
    const dir = path.dirname(outputPath);
    if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true });
    
    fs.writeFileSync(outputPath, JSON.stringify(validProblems, null, 2));
